...
def simulate_w_fixed_points(test_charge, charges, time):
    tstep = 0.001
    n = int(time / tstep)
    for _ in range(n):
        test_charge.pos += test_charge.velocity * tstep
        F = test_charge.q * charge.Efield(test_charge.pos, charges)
...
def decipher(code):
    ...
    for i in range(len(m)):
        charges.append(charge.Charge(m[i], q[i], pos[i]))
    return charges
