...
def EfieldGraph(charges):
    ...
    def magnitude(magnitudes, min_mag, max_mag):
        return 0.002 + 0.004 * ((mag - min_mag) / (max_mag - min_mag))
    ...
    for i in range(n):
        for j in range(n):
            ...
            ax.quiver(
                X[j, i], Y[j, i],
                U[j, i]/mag, V[j, i]/mag,
                width=magnitude(mag, np.min(magnitudes), np.max(magnitudes)),
                scale = 40
            )
    
    
    #ax.quiver(X, Y, U/magnitude, V/magnitude, magnitude, angles = 'xy', scale = 40)
    ax.set_aspect('equal')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    plt.show()